...
public class MainActivity extends AppCompatActivity {
    ...
    public void consultarUsuario(View view){
        ...
        if(!document.isEmpty()){
            Cursor cursor = database.rawQuery("SELECT " + DBHelper.COL_CEDULA + "," + DBHelper.COL_NOMBRE + ", " + DBHelper.COL_TELEFONO + ", " + DBHelper.COL_SOLICITUD + " FROM " + DBHelper.TABLE_USUARIO + " WHERE " + DBHelper.COL_CEDULA + " = ?", new String[]{document});
            if (cursor.moveToFirst()){
                // Asignar los datos de consulta al TextView
                String resultado = "Cédula: "+ cursor.getString(0) + "\nNombre: " + cursor.getString(1) + "\nTeléfono: " + cursor.getString(2) + "\nSolicitud: " + cursor.getString(3);
